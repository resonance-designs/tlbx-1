...
use util::channel_layout::mask_from_av_layout;
...
impl Audio {
    ...
    pub fn channel_layouts(&self) -> Option<ChannelLayoutIter> {
        unsafe {
            if (*self.codec.as_ptr()).ch_layouts.is_null() {
                None
            } else {
                Some(ChannelLayoutIter::new(
                    (*self.codec.as_ptr()).ch_layouts,
                ))
            }
        }
    }
...
impl ChannelLayoutIter {
    pub fn new(ptr: *const AVChannelLayout) -> Self {
...
impl Iterator for ChannelLayoutIter {
    ...
    fn next(&mut self) -> Option<<Self as Iterator>::Item> {
        unsafe {
            if (*self.ptr).nb_channels == 0 {
                return None;
            }

            let layout = ChannelLayout::from_bits_truncate(mask_from_av_layout(&*self.ptr));
            self.ptr = self.ptr.offset(1);

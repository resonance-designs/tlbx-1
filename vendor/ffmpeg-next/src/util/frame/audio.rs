...
use util::format;
use ChannelLayout;
use util::channel_layout::{mask_from_av_layout, to_av_layout};
...
impl Audio {
    ...
    pub fn channel_layout(&self) -> ChannelLayout {
        unsafe { ChannelLayout::from_bits_truncate(mask_from_av_layout(&(*self.as_ptr()).ch_layout)) }
    }

    #[inline]
    pub fn set_channel_layout(&mut self, value: ChannelLayout) {
        unsafe { (*self.as_mut_ptr()).ch_layout = to_av_layout(value) }
    }

    #[inline]
    pub fn channels(&self) -> u16 {
        unsafe { (*self.as_ptr()).ch_layout.nb_channels as u16 }
    }

    #[inline]
    pub fn set_channels(&mut self, value: u16) {
        unsafe {
            let mut layout = to_av_layout(ChannelLayout::from_bits_truncate(0));
            av_channel_layout_default(&mut layout, i32::from(value));
            (*self.as_mut_ptr()).ch_layout = layout;
        }
    }
